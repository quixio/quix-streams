...
from quixstreams.state.windows import get_window_range, TumblingWindow, WindowResult, TumblingWindowDefinition
from tests.utils import TopicPartitionStub


class TestTumblingWindow:

    @pytest.mark.parametrize("timestamp, duration, expected", [
        (100, 10, (100, 109.9)),
        (100, 60, (60, 119.9)),
    ])
    ...
    @pytest.mark.parametrize("duration, grace, provided_name, func_name, expected_name", [
        (10, 5, "custom_window", "sum", "custom_window"),
        (10, 5, None, "sum", "tumbling_window_10_sum"),
        (15, 5, None, "count", "tumbling_window_15_count"),
        # Add more test cases if needed
    ])
    def test_tumbling_window_definition_get_name(self, duration, grace, provided_name, func_name, expected_name):
        twd = TumblingWindowDefinition(duration=duration, grace=grace, dataframe=None, name=provided_name)
    ...
    @pytest.mark.parametrize("duration, grace, name", [
    ...
    @pytest.mark.parametrize("timestamp, latest_timestamp, expired_windows, expected_output", [
        # Case: stale timestamp
        (100, 110, [{}], ([], [])),
        # Case: fresh timestamp
        (100, 90, [{}], ([{"value": "test_value", "start": 90, "end": 100}], [])),
    ])
    def test_tumbling_window_process_window(self, timestamp, latest_timestamp, expired_windows, expected_output):
        state_mock = MagicMock(spec=WindowedTransactionState)
