...
from quixstreams.state.metadata import CHANGELOG_PROCESSED_OFFSET_MESSAGE_HEADER
...
class TestStorePartitionChangelog:
    ...
    def test_recover_from_changelog_message_missing_cf_headers(self, store_partition):
        changelog_msg = ConfluentKafkaMessageStub(
            key=b'my_key|"count"',
            value=b"10",
            headers=[],
        ...
        with pytest.raises(ColumnFamilyHeaderMissing):
            store_partition.recover_from_changelog_message(
                changelog_msg, committed_offset=-1001
            )
