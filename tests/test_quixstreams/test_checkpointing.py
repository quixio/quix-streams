...
from quixstreams.checkpointing import InvalidStoredOffset
...
class TestCheckpoint:
    ...
    def test_commit_with_state_no_changelog_success(
        ...
        state_manager = state_manager_factory(producer=row_producer_mock)
        checkpoint = checkpoint_factory(
            consumer_=consumer,
            state_manager_=state_manager,
            producer_=row_producer_mock,
        )
        processed_offset = 999
        ...
        assert row_producer_mock.flush.call_count == 1
    ...
    def test_close_no_offsets(
        ...
        if exactly_once:
            ...
            assert row_producer_mock.abort_transaction.call_count
        else:
            assert not row_producer_mock.abort_transaction.call_count
    ...
    def test_commit_has_failed_transactions_fails(
        ...
        assert not row_producer_mock.flush.call_count

        # Check nothing is committed
        assert not row_producer_mock.commit_transaction.call_count
    ...
    def test_commit_producer_flush_fails(
        ...
        row_producer_mock.flush.side_effect = ValueError("Flush failure")
    ...
    def test_commit_consumer_commit_fails(
        ...
        assert row_producer_mock.flush.call_count
        # The transaction should remain prepared, but not completed
        assert tx.prepared
        assert not tx.completed
    ...
    @pytest.mark.parametrize("row_producer_mock", [1], indirect=True)
    def test_incomplete_flush(
        self, checkpoint_factory, consumer, state_manager_factory, row_producer_mock
    ):
        state_manager = state_manager_factory(producer=row_producer_mock)
        checkpoint = checkpoint_factory(
            consumer_=consumer,
    ...
    def test_failed_commit(
        self, checkpoint_factory, state_manager_factory, row_producer_mock
    ):
        consumer_mock = MagicMock(spec_set=Consumer)
