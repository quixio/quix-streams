...
namespace Quix.Streams.Streaming.Models
{
    ...
    public class TimeseriesBufferConfiguration
    {
        ...
        /// of data the <see cref="TimeseriesBuffer.OnReceived"/> event is invoked and the data is cleared from the buffer.
        ...
        /// earliest and latest buffered timestamp surpasses this number the <see cref="TimeseriesBuffer.OnReceived"/> event
        ...
        /// <summary>
        /// Gets or set the custom function which is invoked before adding the timestamp to the buffer. If returns true, <see cref="TimeseriesBuffer.OnReceived"/> is invoked before adding the timestamp to it.
        /// Defaults to null (disabled).
        /// </summary>
        public Func<TimeseriesDataTimestamp, bool> CustomTriggerBeforeEnqueue { get; set; } = null;

        /// <summary>
        /// Gets or sets the custom function which is invoked after adding a new timestamp to the buffer. If returns true, <see cref="TimeseriesBuffer.OnReceived"/> is invoked with the entire buffer content
        /// Defaults to null (disabled).
        /// </summary>
        ...
        /// <summary>
        /// Gets or sets the maximum duration in milliseconds for which the buffer will be held before triggering <see cref="TimeseriesBuffer.OnReceived"/> event. 
        /// <see cref="TimeseriesBuffer.OnReceived"/> event is triggered when the configured <see cref="BufferTimeout"/> has elapsed from the last data received by the buffer.
        /// Defaults to null (disabled). 
        /// </summary>
