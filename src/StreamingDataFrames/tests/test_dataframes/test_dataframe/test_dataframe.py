...
class TestDataframeProcess:
    ...
    def test_column_subset(self, dataframe, row_factory):
        dataframe = dataframe[["x", "y"]]
        row = row_factory({"x": 1, "y": 2, "z": 3})
        expected = row_factory({"x": 1, "y": 2})
        assert dataframe.process(row) == expected

    def test_column_subset_with_funcs(self, dataframe, row_factory, row_plus_n_func):
        dataframe = dataframe[["x", "y"]].apply(row_plus_n_func(n=5))
        row = row_factory({"x": 1, "y": 2, "z": 3})
        expected = row_factory({"x": 6, "y": 7})
        assert dataframe.process(row) == expected

    def test_inequality_filter(self, dataframe, row_factory):
        dataframe = dataframe[dataframe["x"] > 0]
        row = row_factory({"x": 1, "y": 2})
        assert dataframe.process(row) == row

    def test_inequality_filter_is_filtered(self, dataframe, row_factory):
        dataframe = dataframe[dataframe["x"] >= 1000]
        row = row_factory({"x": 1, "y": 2})
        assert dataframe.process(row) is None

    def test_inequality_filter_with_operation(self, dataframe, row_factory):
        dataframe = dataframe[(dataframe["x"] - 0 + dataframe["y"]) > 0]
        row = row_factory({"x": 1, "y": 2})
        assert dataframe.process(row) == row

    def test_inequality_filter_with_operation_is_filtered(self, dataframe, row_factory):
        dataframe = dataframe[(dataframe["x"] - dataframe["y"]) > 0]
        row = row_factory({"x": 1, "y": 2})
        assert dataframe.process(row) is None

    def test_inequality_filtering_with_apply(self, dataframe, row_factory):
        dataframe = dataframe[dataframe["x"].apply(lambda v: v - 1) >= 0]
        row = row_factory({"x": 1, "y": 2})
        assert dataframe.process(row) == row

    def test_inequality_filtering_with_apply_is_filtered(self, dataframe, row_factory):
        dataframe = dataframe[dataframe["x"].apply(lambda v: v - 10) >= 0]
        row = row_factory({"x": 1, "y": 2})
        assert dataframe.process(row) is None

    def test_compound_inequality_filter(self, dataframe, row_factory):
        dataframe = dataframe[(dataframe["x"] >= 0) & (dataframe["y"] < 10)]
        row = row_factory({"x": 1, "y": 2})
        assert dataframe.process(row) == row

    def test_compound_inequality_filter_is_filtered(self, dataframe, row_factory):
        dataframe = dataframe[(dataframe["x"] >= 0) & (dataframe["y"] < 0)]
        row = row_factory({"x": 1, "y": 2})
        assert dataframe.process(row) is None
    ...
    def test_non_row_apply_breaks_things(self, dataframe, row_factory):
        dataframe = dataframe.apply(lambda row: False)
        dataframe = dataframe.apply(lambda row: row)
        row = row_factory({"x": 1, "y": 2})
        dataframe.process(row)

    def test_multiple_row_generation(self, dataframe, more_rows_func, row_factory):
    ...
    def test_multiple_row_generation_with_additional_apply(
        self, dataframe, more_rows_func, row_factory, row_plus_n_func
    ):
        dataframe = dataframe.apply(more_rows_func)
        dataframe = dataframe.apply(row_plus_n_func(n=1))
    ...
    def test_multiple_row_generation_with_additional_filtering(
        self, dataframe, more_rows_func, row_factory
    ):
        dataframe = dataframe.apply(more_rows_func)
...
class TestDataframeKafka:
    ...
    def test_to_topic_no_producer_assigned(self, dataframe, row_factory):
