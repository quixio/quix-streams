...
**This library is in alpha and will experience rapid and significant interface changes, 
feature additions, and bugfixes. Use with caution!**
...
# The Basics of `StreamingDataFrame`

So, now you want to get started with `StreamingDataFrame`...what do you need to know?

More configuration details and options can be found in the **Advanced Usage** section.
...
If you are running this directly in the Quix platform it will be configured 
...
NOTE: There are many different ways to serialize your data with kafka, and 
often you will provide a serializer for a topic. See the **Serialization** section for
more details.
...
Finally, we want to run all this as a Kafka client by handing the sdf to the `.run`
call:
...
Once running it will, as an endless loop,
- consume a message
...
Along with a `StreamingDataFrame` instance similar to one generated with 
...
value, where the dictionary key is equivalent to a dataframe column name. 
...
### Referencing fields/columns

In typical Pandas dataframe fashion, you can reference a column:
...
You can also reference nested objects (dicts, lists, etc):

```python
sdf["field_c"][2]  # 3
```

### Performing operations with (referenced) columns

In typical Pandas dataframe fashion, you can do almost any basic operations or 
comparisons with referenced columns, assuming validity:
...
See `.apply()` details for more information on how that works.


### Subsetting/slicing column sets
...
It is a way to effectively skip ALL additional SDF-defined processes for that row,
_including kafka-related operations like producing_.

Basically, when a "column" reference is a boolean value, it will be treated as a 
"filtering" operation, where anything but `True` will result in "filtering".
