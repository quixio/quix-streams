...
class StreamReader(object):
    ...
    def __init__(self, net_pointer: ctypes.c_void_p, input_topic: 'StreamReader', on_close_cb_always: Callable[['StreamReader'], None]):
        """
    ...
    @property
    def on_stream_closed(self) -> Callable[['InputTopic', 'StreamReader', 'StreamEndType'], None]:
        """
    ...
    def on_stream_closed(self, value: Callable[['InputTopic', 'StreamReader', 'StreamEndType'], None]) -> None:
        """
    ...
    def _on_stream_closed_wrapper(self, sender_hptr, args_hptr):
        ...
        try:
            with (args := StreamClosedEventArgs(args_hptr)):
                converted = ec.enum_to_another(args.get_EndType(), StreamEndType)
                self.on_stream_closed(self._input_topic, self, converted)
            InteropUtils.free_hptr(sender_hptr)
        except:
            traceback.print_exc()
    ...
    @property
    def on_package_received(self) -> Callable[['InputTopic', 'StreamReader', any], None]:
        """
    ...
    def on_package_received(self, value: Callable[['InputTopic', 'StreamReader', any], None]) -> None:
        """
    ...
    def properties(self) -> StreamPropertiesReader:
        ...
        if self._streamPropertiesReader is None:
            self._streamPropertiesReader = StreamPropertiesReader(self._input_topic, self, self._interop.get_Properties())
    ...
    def events(self) -> StreamEventsReader:
        ...
        if self._streamEventsReader is None:
            self._streamEventsReader = StreamEventsReader(self._input_topic, self, self._interop.get_Events())
    ...
    def parameters(self) -> StreamParametersReader:
        ...
        if self._streamParametersReader is None:
            self._streamParametersReader = StreamParametersReader(self._input_topic, self, self._interop.get_Parameters())
