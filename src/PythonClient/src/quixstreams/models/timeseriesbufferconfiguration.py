...
class TimeseriesBufferConfiguration(object):
    ...
    When none of the buffer conditions are configured, the buffer immediate invokes the on_receive
    """
    ...
    def buffer_timeout(self) -> Optional[int]:
        """
            ...
            on_receive event is triggered when the configured value has elapsed or other buffer condition is met.
            Defaults to none (disabled).
        """
    ...
    def custom_trigger_before_enqueue(self) -> Callable[[TimeseriesDataTimestamp], bool]:
        """
            Gets the custom function which is invoked before adding the timestamp to the buffer. If returns true, TimeseriesBuffer.on_receive is invoked before adding the timestamp to it.
            Defaults to none (disabled).
        """
    ...
    def custom_trigger_before_enqueue(self, value: Callable[[TimeseriesDataTimestamp], bool]):
        """
            Sets the custom function which is invoked before adding the timestamp to the buffer. If returns true, TimeseriesBuffer.on_receive is invoked before adding the timestamp to it.
            Defaults to none (disabled).
        """
