...
from src.quixstreams.dataframes.kafka import (
    QuixKafkaConfigsFactory,
    QuixPortalApiWrapper,
)
...
def mock_quix_portal_api_factory(mock_response_factory):
    def mock_quix_portal_api(api_responses: dict = None):
        if not api_responses:
            api_responses = {}
        api_obj = create_autospec(QuixPortalApiWrapper)
        api_obj.default_workspace_id = None
        for call, json in api_responses.items():
            api_obj.__getattribute__(call).return_value = mock_response_factory(json)
        return api_obj
...
def quix_kafka_config_factory(mock_quix_portal_api_factory):
    def mock_quix_kafka_configs(
        ...
        return QuixKafkaConfigsFactory(
            quix_portal_api_wrapper=api_class,
