...
class TestApi:
    # TODO: test init, get, post maybe?
    ...


class TestQuixKafkaConfigsFactory:
    ...
    def test_search_for_topic_workspace_no_topic(self, quix_kafka_config_factory):
        ...
        cfg_factory = quix_kafka_config_factory(
            api_responses={"get_workspaces": api_response}
        )
    ...
    def test_get_workspace_ssl_cert(self, quix_kafka_config_factory, tmp_path):
        ...
        cfg_factory = quix_kafka_config_factory(
            workspace_id="12345",
            api_responses={"get_workspace_certificate": zip_in_mem.getvalue()}
        )
    ...
    def test_get_confluent_broker_config(self, quix_kafka_config_factory):
        cfg_factory = quix_kafka_config_factory(
            workspace_id="12345",
            workspace_cert_path="/mock/dir/ca.cert"
        )
    ...
    def test_get_consumer_config(self, quix_kafka_config_factory):
        cfg_factory = quix_kafka_config_factory(workspace_id="12345")
        topics = ["topic_1", "topic_2"]
        group_id = "my_consumer_group"
        with patch.object(
                cfg_factory,
                "get_confluent_broker_config",
                return_value={"cfgs": "here"}
        ) as cfg:
            result = cfg_factory.get_consumer_config(topics, group_id)
