...
1. adding a new set of operation(s) to an `SDF` instance that you wish to branch from.
    ...
    - These operation(s) should _**NOT**_ begin with an [in-place operation](./advanced/dataframe-assignments.md#valid-in-place-operations) (such as `.update()`, column assignment `sdf['col'] = ...`, or `.drop()`). Starting a branch with an in-place operation modifies the parent branch rather than creating a new independent one.
