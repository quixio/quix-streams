...
If a rebuild encounters an interrupted migration (some records already stamped, some not — a "MIXED" changelog), recovery auto-completes the remaining un-stamped records. With `legacy_records_ttl` set, they expire at the **later** of `wallclock-at-rebuild + legacy_records_ttl` and the surviving stamped cohort's own expiry — so they never expire before the records they were migrated alongside. (The sweep expires records on the event-time clock, so a purely wall-clock expiry would grant them `legacy_records_ttl` *minus* any event-time skew — negative, and hence immediate deletion, on a store whose event-time runs ahead of the rebuilding node.) without it, they inherit the expiry of the already-stamped cohort (or `SENTINEL_NEVER` with a `[WARNING]` in the unlikely case that no stamped record with a future expiry survives). No data is deleted.
...
- **Cold rebuild / fresh volume / memory backend.** The only durable trace is the raw stamped bytes, which are byte-identical to a legacy `set_bytes()` value. When every replayed key decodes as a valid 8-byte stamp with a non-empty value payload, recovery adopts the stamps **provisionally and reversibly**: it flips the store into TTL mode, keeps every value verbatim (original stamps preserved), rebuilds the expiry index, and **backs up the originals** to a local `__ttl_adopt_backup__` column family. While provisional, the TTL sweep is suppressed so no misidentified record can be deleted. A `[WARNING]` (not a `CRITICAL`) records the reversible adoption. The adoption is **corroborated** — made permanent, backup dropped, sweep re-enabled — the first time the store sees a genuine live `state.set(..., ttl=...)` write, which also produces a durable migration-done marker so future cold rebuilds are deterministic.

A census is refused if any of its values are bare 8-byte stamps with no payload — the shape of a legacy `set_bytes()` dedup store, which stores a plain epoch-ms timestamp as the entire value. Those values are structurally indistinguishable from a TTL stamp at the byte level but carry no application payload, so the evidence rule excludes them. The store stays legacy, every value reads back byte-identical, and a `[WARNING]` explains the refusal. An all-past census (every stamp already expired) is still adopted — liveness is not part of the evidence rule — and the sweep reclaims the expired records after corroboration lifts sweep suppression.
...
Rolling back an adoption that was in fact *correct* is safe but not silent: the untouched originals of a genuine v3.24.0 store are real expiry stamps, so once the store is legacy again a read of one raises `StateMigrationError` naming `ttl_force_flip` rather than handing stamped bytes to the value deserializer. Clear the rollback lever (or set the force-flip one) and restart to put that store back into TTL mode.

**Repairing a store whose TTL flag went missing.** A store can end up holding TTL-stamped values while its local `__ttl_enabled__` flag is absent — a migration whose process died before it could record the flip, or a rollback that cleared the flag while leaving the (genuinely stamped) values byte-identical. Opened in legacy mode, every read of such a value used to fail in the value deserializer, once per restart, with no recovery pass to fix it. This is now repaired automatically at open: when the store still carries migration bookkeeping **and** a bounded sample of its values decodes unanimously as TTL stamps (non-empty payloads), the framework flips it back into TTL mode, persists the flip, and lets the normal recovery pass finish the migration with the data partitions paused. A store with no such evidence is left untouched and byte-identical — including a legacy `set_bytes()` dedup store whose 8-byte timestamps merely look like stamps.

If the evidence is gone (a rebuilt state directory, cleaned-up bookkeeping) the repair cannot identify the store, and a read of a stamped value raises `StateMigrationError` naming the way out: set `ttl_force_flip=True` on `RocksDBOptions` (or `QUIXSTREAMS_STATE_TTL_FORCE_FLIP=1`) and restart to force the flip, or clear the partition's state and let it rebuild from the changelog. The lever is a no-op once the store is flipped, so it is safe to leave set for one restart and then clear.

**Both levers are settable in code.** `RocksDBOptions(ttl_rollback=True)` and `RocksDBOptions(ttl_force_flip=True)` are equivalent to the two environment variables, and each lever is on when *either* surface is set (the option wins; the environment variable is consulted otherwise). Prefer the options in Quix Cloud: a deployment environment variable that the application's `app.yaml` does not declare is silently dropped on redeploy, so an env-only lever can disappear between runs. Each partition logs which surface turned a lever on (`TTL rollback lever: ON (source=options)`), and setting both levers at once is rejected — as a `ValueError` at `RocksDBOptions` construction, or at store open when an option and an environment variable collide.

The memory backend is always the cold regime (it re-recovers from the changelog on every open). It mirrors the same automatic provisional-adopt / rollback behavior with an in-RAM backup; a fresh replay reconstructs the un-adopted originals, so `QUIXSTREAMS_STATE_TTL_ROLLBACK=1` (or `MemoryStore(..., ttl_rollback=True)`) simply suppresses the auto-adopt on the next open. It accepts `ttl_force_flip` for configuration parity only: it persists no flip flag, so there is nothing for that lever to repair.

For the expected log output for each v3.24.0 upgrade path, the two operator levers, and changelog topic sizing for short TTLs, see [Upgrading from v3.24.0](state-ttl.md#upgrading-from-v3240) in the cookbook.
...
**Cold-restore completion.** If a rebuilt node replays a changelog whose migration was interrupted mid-backfill (some records already stamped, some not), it auto-completes the leftovers at end of recovery. With `legacy_records_ttl` set, they expire at the **later** of `wallclock-at-rebuild + legacy_records_ttl` and the surviving stamped cohort's own expiry — so they never expire before the records they were migrated alongside. (The sweep expires records on the event-time clock, so a purely wall-clock expiry would grant them `legacy_records_ttl` *minus* any event-time skew — negative, and hence immediate deletion, on a store whose event-time runs ahead of the rebuilding node.) without it, they inherit the expiry of the surviving stamped cohort (or never-expire, with a `[WARNING]`, in the rare case where no surviving stamp remains to derive from). No data is deleted.
...
```
[INFO] Recovery: completing interrupted legacy-TTL migration at path=<...>; <N> leftover legacy record(s) will be stamped with expiry=<...> (basis: <max-surviving-cohort-stamp|wallclock-at-rebuild + legacy_records_ttl>; the later of wallclock+legacy_records_ttl=<...> and the max surviving cohort stamp <...>, so the leftovers expire WITH their cohort and never before it).
...
A census is refused when any value fails the evidence rule — bare 8-byte payloads (the legacy `set_bytes()` dedup shape) or a mixed sample without a crashed-completion signature on disk. In those cases the store stays legacy and the log shows:

```
[WARNING] ... NOT flipping it: on a genuinely legacy store that would strip 8 bytes off every read and let the sweep delete every record. ... If this store really is TTL-migrated, set QUIXSTREAMS_STATE_TTL_FORCE_FLIP=1 and restart to force the flip.
```

`QUIXSTREAMS_STATE_TTL_ROLLBACK=1` reverses a provisional (uncorroborated) adoption while its backup exists. See [Reference clock — when do migrated records expire?](#reference-clock-when-do-migrated-records-expire) for the adoption rationale.
