...
Also being introduced alongside branching is automated `StreamingDataFrame` tracking.
...
While most functionality works with `branching`, there is one situation to be aware of.


## Custom State (`stateful=True`)

Performing `StreamingDataFrame` operations that use `stateful = True` is something to
be careful with, as the operations will share the same state between one another.

For example, assume:
- two branches with same origin: `branch_A` and `branch_B` 
  - each use a `.apply(f, stateful = True)` (unique `f` per branch)
...
This "shared" state concern is NOT applicable across `SDF`'s used for 
...
## Branching and Data Cloning

Any nodes with branches require cloning the input current value at that node `N-1` 
...
Though unlikely, if encountered, try changing the message value to a different 
format before the branching occurs.
...
operations (starting from the pruned node), which is then treated as the actual 
operation being applied.

Pruning occurs during filtering and column assignment operations, and are only allowed
when when two nodes have the same origin. Why?

Pruned operations are entirely autonomous, as if steps from a recipe were cut 
and pasted into another with no edits: they are a context-less collection of functions.
