...
class WindowedRocksDBPartitionTransaction(PartitionTransaction):
    ...
    def expire_all_windows(
        ...
        windows = expiring_window_end(last_expired, max_end_time, step_ms)
        if not windows:
            return

        suffixes = tuple(int_to_int64_bytes(window_end) for window_end in windows)
        ...
        self._set_timestamp(
            prefix=b"", cache=self._last_expired_timestamps, timestamp_ms=windows[-1]
        )
