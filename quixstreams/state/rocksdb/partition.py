...
class _StampEvidence(NamedTuple):
    ...
    - ``stamped`` — values that ARE TTL stamps (in-window expiry or the
      ...
      gates on, via :attr:`unanimously_stamped`;
...
class RocksDBStorePartition(StorePartition):
    ...
    def __init__(
        ...
        # unexplainable afterwards (this happened while diagnosing sc-74843).
        ...
        # (its default CF does not sample as uniformly stamped), or because the
        # rollback lever asserted legacy intent. Such a store is in an
        # unreconciled state:
        ...
        # left the rolled-back store unguarded and reproduced the sc-74843
        # deserializer crash on the very lever meant to be the way out
        # (Tester round 1, bug 1.1, 2026-09-04).
        #
        ...
        # does NOT count. That case proves "these bytes are stamped", never "this
        # store was mid legacy->TTL migration", and the difference decides how
        # ``complete_recovery`` treats the census: with this False the census is
        # re-judged from its own bytes by BRANCH B's survey, which additionally
        # requires FULL default-CF coverage before it flips read semantics
        # store-wide and adopts the census VERBATIM (backup + sweep-guard, no
        # re-wrap). The residual is accepted and bounded: a repaired store whose
        # census holds genuine un-stamped leftovers has them discarded rather
        # than stamped, so they stay readable (the fail-safe read path returns an
        # un-stamped value raw) and never-expiring instead of being corrupted.
    ...
    def _has_warm_ttl_artifacts(self) -> bool:
        ...
        crash-looped on the first read.
    ...
    def _repair_unflagged_stamped_store(self, artifacts: str, forced: bool) -> bool:
        ...
        **Evidence gate: UNANIMITY, not liveness.** ``forced=False`` (the
        ...
        if not forced and not evidence.unanimously_stamped:
            logger.warning(
                ...
                "value reads back byte-identical. If this store really is "
    ...
    def has_incomplete_ttl_migration(self) -> bool:
        ...
        - the partition is flipped into TTL mode (``uses_ttl_stamps`` loaded True
          ...
          data partitions paused (sc-74843);
